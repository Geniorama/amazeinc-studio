          ...
          <div className="container">
              ...
              {data
                ...
                <div className={styles.contCategories}>
                  <ul className={styles.contItemsCategories}>
                    {data.categories.data.directorsProjects.nodes.map((cat) => {
                      if(cat.count != null && cat.count >=1){
                        return (
                          <li key={cat.slug}>
                            <Link href={`/projects/director/${cat.slug}`}>
            ...
            <div>
              ...
              <motion.div key={"grid-projects"} className={styles.gridProjects} initial={'hide'} animate={!isLoading ? 'show' : 'hide'} variants={variants} transition={{delay: 0, duration: 1}}>
                  {data.projects.data.directorProjects.translation.projects.nodes.map((item) =>(
                    item.featuredImage
                    ?
...
export async function getServerSideProps({ locale, params, req, res }) {
  ...
  try {
    ...
    const categories = await queries.getDirectorsProjects(API_URL, localeCovert(locale))
    const projects = await queries.getProjectsByDirector(API_URL, localeCovert(locale), slug)
